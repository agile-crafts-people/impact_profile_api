"""
E2E tests for Platform endpoints.

These tests verify that Platform endpoints work correctly by making
...
def test_create_platform_endpoint():
    """Test POST /api/platform endpoint and verify record persists in database."""
    ...
    data = {
        ...
        "description": "E2E test platform document",
    }

    response = requests.post(f"{BASE_URL}/api/platform", headers=headers, json=data)
    assert response.status_code == 201, _err(response, 201)

    response_data = response.json()
    assert "_id" in response_data, "Response missing '_id' key"
    assert response_data["name"] == "e2e-test-platform"
    assert "created" in response_data
    assert "saved" in response_data


@pytest.mark.e2e
def test_get_platforms_endpoint():
    """Test GET /api/platform endpoint."""
    token = get_auth_token()
    assert token is not None, "Failed to get auth token"
...
def test_get_platforms_with_name_filter():
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/platform?name=e2e", headers=headers)
    assert response.status_code == 200, _err(response, 200)
...
def test_platform_endpoints_require_auth():
    ...
    response = requests.get(f"{BASE_URL}/api/platform")
